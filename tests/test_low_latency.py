...
def test_main(num_tokens: int,
    ...
    use_coop = os.environ.get('DEEPEP_COOP', '0') == '1'
    do_check = not use_coop  # Skip detailed correctness checks for cooperative kernel path (WIP)
    hash_value, num_times = 0, 0
    # For coop path, set current_x for bench section; skip correctness loop
    current_x = x_list[0]
    for current_x in ([] if use_coop else x_list):
    ...
    # For coop path: create simulated_gemm_x since correctness loop was skipped
    if use_coop:
        num_local_experts = num_experts // num_ranks
        # The dispatch_recv kernel assigns padded_index = 0, 1, 2, ..., coop_total_recv-1.
        # The combine_send kernel indexes expert_x_ptr with these padded_index values.
        # So the expert output tensor must have at least max_possible_recv rows.
        # max_recv = num_tokens * num_topk * num_ranks is the upper bound.
        ...
        simulated_gemm_x = torch.randn(max_recv_for_buf, hidden,
    ...
    for return_recv_hook in (False, True):
        group.barrier()
        # Iter 61: Reset tag counters between bench phases to prevent tag wrapping
        # past kMaxTags (1024). Safe after barrier since no coop ops are in-flight.
        if use_coop and hasattr(buffer.runtime, 'coop_reset_tags'):
            buffer.runtime.coop_reset_tags()
