...
def test_main(args: argparse.Namespace,
    ...
    # Test dispatch
    # noinspection PyShadowingNames
    def check_data(check_x, recv_gbl_rank_prefix_sum, label=""):
        row_min = check_x.amin(dim=1)
        row_max = check_x.amax(dim=1)
        if not torch.allclose(row_min, row_max):
            bad_rows = (row_min != row_max).nonzero(as_tuple=True)[0]
            print(f'[check_data {label}] FAIL: {bad_rows.numel()} rows not constant. '
                  f'First bad row={bad_rows[0].item()}, first 8 vals={check_x[bad_rows[0], :8].tolist()}',
                  flush=True)
            assert False, f"check_data row-constness failed at {label}"
        check_start = 0
        for i in range(num_ranks):
            check_end = recv_gbl_rank_prefix_sum[i].item()
            diff_sum = (check_x[check_start:check_end, :].int() - i).sum().item()
            if diff_sum != 0:
                actual_vals = check_x[check_start, :8].tolist() if check_end > check_start else []
                print(f'[check_data {label}] FAIL at rank i={i}: '
                      f'check_start={check_start}, check_end={check_end}, diff_sum={diff_sum}, '
                      f'first_row_vals={actual_vals}', flush=True)
                assert False, f"check_data value mismatch at rank {i}, label={label}"
        ...
        for async_mode in (False, True):
            for current_x in (x_pure_rand, x, x_pure_rand_e4m3, x_e4m3):
                for with_topk in (False, True):
                    ...
                    if not is_rand:
                        check_data(recv_x, recv_gbl_rank_prefix_sum, "fresh_dispatch")
                    ...
                    if with_topk:
                        ...
                        if not is_rand:
                            ...
                            check_data(recv_topk_weights, recv_gbl_rank_prefix_sum, "fresh_topk_weights")
                    ...
                    if with_topk:
                        ...
                        if not torch.equal(recv_x, recv_worst_x[:recv_x.size(0)]):
                            diff_mask = recv_x != recv_worst_x[:recv_x.size(0)]
                            diff_rows = diff_mask.any(dim=1).nonzero(as_tuple=True)[0]
                            print(f'[rank {rank}] num_worst_tokens recv_x mismatch: '
                                  f'{diff_rows.numel()}/{recv_x.size(0)} rows differ, '
                                  f'first bad rows={diff_rows[:5].tolist()}, '
                                  f'recv_x[{diff_rows[0].item()},:8]={recv_x[diff_rows[0],:8].tolist()}, '
                                  f'recv_worst_x[{diff_rows[0].item()},:8]={recv_worst_x[diff_rows[0],:8].tolist()}',
                                  flush=True)
                            assert False, f"rank {rank}: recv_x != recv_worst_x"
                        if not torch.equal(recv_topk_idx, recv_worst_topk_idx[:recv_x.size(0)]):
                            diff_mask = recv_topk_idx != recv_worst_topk_idx[:recv_x.size(0)]
                            diff_rows = diff_mask.any(dim=1).nonzero(as_tuple=True)[0]
                            print(f'[rank {rank}] num_worst_tokens topk_idx mismatch: '
                                  f'{diff_rows.numel()}/{recv_topk_idx.size(0)} rows differ, '
                                  f'first bad rows={diff_rows[:5].tolist()}', flush=True)
                            assert False, f"rank {rank}: recv_topk_idx != recv_worst_topk_idx"
                        if not torch.equal(recv_topk_weights_clone, recv_worst_topk_weights[:recv_x.size(0)]):
                            diff_mask = recv_topk_weights_clone != recv_worst_topk_weights[:recv_x.size(0)]
                            diff_rows = diff_mask.any(dim=1).nonzero(as_tuple=True)[0]
                            print(f'[rank {rank}] num_worst_tokens topk_weights mismatch: '
                                  f'{diff_rows.numel()}/{recv_topk_weights_clone.size(0)} rows differ',
                                  flush=True)
                            assert False, f"rank {rank}: recv_topk_weights != recv_worst_topk_weights"
                    ...
                    if not with_topk:
                        ...
                        recv_x = per_token_cast_back(*recv_x) if isinstance(recv_x, tuple) else recv_x
                        if not is_rand:
                            check_data(recv_x, recv_gbl_rank_prefix_sum, "cached_dispatch")
