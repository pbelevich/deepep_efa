...
def bench_kineto(fn,
    ...
    original_kernel_names = kernel_names
    ...
    # Check if all kernel names are present in profiling output.
    # For EFA/NCCL-based internode implementation, the custom CUDA kernel names
    # (e.g. 'dispatch', 'notify', 'combine') won't appear — fall back to
    # wall-clock timing using CUDA events so the benchmark numbers are still
    # comparable (they measure end-to-end latency including CPU overhead).
    kernels_missing = any(sum([name in line for line in prof_lines]) != 1 for name in kernel_names)

    if kernels_missing:
        # Fall back: measure with CUDA events (wall-clock)
        torch.cuda.synchronize()
        cache = torch.empty(int(256e6 // 4), dtype=torch.int, device='cuda')
        # Warmup
        for _ in range(10):
        ...
        avg_time = float(np.average(times[1:]))
        # Return the same wall-clock time for every requested kernel name
        # (we can't separate notify vs dispatch in NCCL mode)
        kernel_durations = [avg_time for _ in kernel_names]
        # Handle num_kernels_per_period > 1: expand each duration into a list
        if num_kernels_per_period > 1:
            kernel_durations = [[avg_time / num_kernels_per_period] * num_kernels_per_period for _ in kernel_names]
